            ...
            if (cursor.getCount() > 0){
                ...
                if (isNetworkAvailable()){
                    try {
                        ...
                        phonetics = bundle1.getString("phonetics");

                        try {
                            String query1 = "SELECT * FROM words WHERE word = '" + word + "'";

                            Cursor cursor2 = MainActivity.database.rawQuery(query1, null);
                            if (cursor2.getCount() <= 0) {
                                ContentValues contentValues = new ContentValues();
                                contentValues.put("word", word);
                                contentValues.put("phonetics", phonetics);
